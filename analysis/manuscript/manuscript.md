...
**Results:** We present **MCseg**, a no-code, browser-based Visium HD analysis platform incorporating MCseg v2 as its segmentation engine. MCseg v2 was developed via AI-autonomous optimisation: over ~80 unsupervised cycles, an AutoResearch agent converged on a 7-pass multi-model ensemble with Voronoi-constrained boundary expansion, improving Panoptic Quality from 0.432 to 0.554 (28% relative improvement) against Xenium single-molecule ground truth in LUAD. In CRC, RNA-based benchmarking across 15 ROIs reveals a "FTC Paradox": Space Ranger's apparent transcript-capture advantage disappears when normalised for cell area (UMI density: 11.7 vs 11.6 UMI/µm², <1% difference), while MCseg v2 achieves superior boundary purity (NED: 0.727 vs 0.712, p=0.026; mutually exclusive co-expression 0.49% vs 0.67%, p=0.030). In CRC tertiary lymphoid structures, MCseg v2 resolved 4 functionally distinct immune subtypes versus Space Ranger's 3 mixed populations, with 44% higher cell yield.

**Availability:** MCseg source code and segmentation pipeline are available at [https://github.com/ddmanyes/MCseg](https://github.com/ddmanyes/MCseg) under the MIT License. All analysis scripts are provided in the Supplementary Materials.
...
2. **CLAHE + Voronoi-constrained expansion** (→ 0.648, +0.33 in one cycle): Voronoi tessellation was identified as allocating inter-cellular space in a physically grounded manner, yielding the largest single-cycle gain.
...
Using Xenium single-molecule resolved cell boundaries as unbiased geometric ground truth across 6 LUAD ROIs (Fig. 2A–C), MCseg v2 outperforms MCseg v1 by +0.122 PQ (0.554 ± 0.064 vs 0.432 ± 0.037, 28% relative improvement), with SQ as the primary driver (0.585 → 0.777, +33%), indicating that Voronoi-expanded boundaries more closely approximate Xenium's cytoplasm-inclusive reference than nuclear-centred radial dilation. The modest RQ decrease (0.734 → 0.711) is an expected consequence of cytoplasm-inclusive cyto3 detection merging some adjacent nuclear instances that nuclei-only MCseg v1 resolves separately—an acceptable trade-off for achieving full cellular boundary coverage. The two highest-scoring ROIs (tumour boundary ROI 1 + tumour core ROI 6) achieve mean PQ = 0.636, with ROI 1 reaching PQ = 0.662.

We further applied MCseg v2 to two biologically demanding LUAD microenvironments. In the normal alveolar region (ROI 10), AT1 (*AGER*⁺/*RTKN2*⁺) and AT2 (*SFTPC*⁺/*SFTPB*⁺/*SFTPA1*⁺) pneumocyte discrimination was evaluated. Geometric coverage of AT1 territory was 22.4% for MCseg v2 versus 22.9% for Xenium (pixel recall = 0.71, IoU = 0.56; Fig. 2E–F). However, only 2.1% of MCseg cells expressed *AGER* or *RTKN2* in the Visium HD count matrix, an 11-fold difference relative to Xenium. This dissociation is attributable to **Visium HD platform-level sparsity**: AT1 cells extend thin cytoplasmic sheets across the alveolar surface in 3D, and even within the nuclear footprint, *AGER* and *RTKN2* mRNA density falls below detection threshold within any single 2 µm bin. Xenium's single-molecule FISH detects individual transcripts without bin aggregation, bridging this gap. MCseg v2 therefore represents the geometric ceiling achievable on this tissue; the RNA deficit is intrinsic to the platform, not a segmentation artefact. For AT2 pneumocytes, cytoplasm-inclusive Voronoi boundaries yield a measurable gain: unsupervised Leiden clustering (res=0.4) identifies a discrete AT2 Pneumocyte cluster (C1, n=326, 30%; Fig. 2D) characterised by uniform *SFTPC* co-expression (100% positive), while RNA-level detection across all cells reaches *SFTPC* 34%, *SFTPB* 22%, *SFTPA1* 12% (combined AT2 positivity 45% vs 39% with MCseg v1), demonstrating improved detection of cytoplasm-enriched transcripts with cytoplasm-inclusive boundaries.

In the pigmented alveolar macrophage zone (ROI 9), Xenium fluorescence detection is prone to quenching by dark carbon deposits [7]; H&E-based MCseg v2 achieves comparable cell counts (8,523 vs 9,694). Because *SPP1*, the canonical immunosuppressive TAM marker, is absent from Xenium's 5K targeted panel, the whole-transcriptome coverage of Visium HD enabled identification of a *SPP1*⁺/*FTH1*⁺ TAM cluster: *SPP1* positivity 17.8%, *FTH1* 55% (vs 23% tissue-wide), *TREM2* 12-fold enriched (1.2% vs 0.1% background)—a phenotype linked to poor prognosis and chemoresistance in LUAD [8,9] that no amount of analytic refinement within Xenium's panel can retrieve.
...
At Leiden resolution 0.5, the two methods differed substantially in resolving power (Fig. 4):

- **MCseg v2** (n = 636 cells) recovered **4 distinct functional populations**: Plasma/B cell (*JCHAIN*⁺), Stroma/Fibroblast (*VIM*⁺), Myeloid/Macrophage (*LYZ*⁺), and Well-differentiated Tumour (*CEACAM5*⁺), each forming distinct UMAP clusters (Supplementary Fig. S10A).
- **SR** (n = 440 cells) yielded **3 populations** with overlapping marker expression (Supplementary Fig. S10B): the largest cluster (n = 256; 58% of all cells) simultaneously co-expressed *IGKC* (5.43), *VIM* (3.74), and *LYZ* (1.32), collapsing B-cell, stromal, and myeloid lineages into a single population.

MCseg v2's 44% higher cell yield (636 vs 440) reflects recovery of cells previously absorbed into SR's over-expanded masks. Spatial marker purity also differed: *JCHAIN* signal was confined within MCseg v2 masks, whereas SR showed *JCHAIN* signal extending into adjacent tumour cells—a spatial manifestation of the NED difference quantified in §3.3. Unmapped regions (vessel lumens, lymphatic sinusoidal spaces, ECM corridors) correspond to areas lacking nuclear H&E morphology, consistent with anatomical structure rather than missed detections. Boundary refinement thus resulted in single-cell populations with distinct transcriptional signatures in this lineage-diverse microenvironment.
...
The MCseg v2 segmentation pipeline and transcript attribution scripts are available at [https://github.com/ddmanyes/MCseg](https://github.com/ddmanyes/MCseg) under the MIT License. All analyses were performed in Python 3.11 (Cellpose 3, Scanpy 1.10, Scipy 1.13).
...
(a–c) MCseg v2 vs MCseg v1 across n = 6 LUAD ROIs (Xenium GT): (a) PQ, (b) SQ, (c) RQ. Paired Wilcoxon. (d) ROI 10 MCseg v2 Leiden cluster map (3 clusters, res=0.4): AT2 Pneumocyte cluster (C1, n=326, 30%) identified by uniform *SFTPC* co-expression (100% positive); remaining cells: Unresolved. Scale bar: 50 µm. (e) AT1 spatial overlay: Xenium GT AT1 cells (cyan) and MCseg v2 boundaries; pixel recall = 0.71, IoU = 0.56. (f) AT1 detection rate comparison: Xenium GT 22.9% ≈ MCseg geometric 22.4% > MCseg RNA 2.1% (11-fold platform gap). (g) ROI 10 Leiden cluster dot plot: AT2 Pneumocyte cluster (C1) shows near-exclusive *SFTPC*/*SFTPB*/*SFTPA1* enrichment; Unresolved clusters (C0, C2) show minimal expression across all lineage markers, confirming Visium HD transcriptomic sparsity as the limiting factor. (h) ROI 9 Leiden cluster map (n = 8,393 cells). (i) ROI 9 dot plot: *SPP1*⁺/*FTH1*⁺ TAM cluster. Scale bars: 50 µm.
