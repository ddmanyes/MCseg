...
MSseg 共 7 個步驟。左側選單顯示進度，前一步驟完成前後續步驟鎖定。
...
### Stage 3.5: 空間基因表達探索 (Spatial Explorer)

**目的**：在完成 Scanpy 分析後，互動式探索基因在空間上的表現模式，直接疊合於 H&E 影像。

#### 顯示模式

| 模式 | 說明 |
| --- | --- |
| **Contour** | 每個細胞以半透明色塊填色 + 深色 inner border 分隔，清楚呈現細胞邊界 |
| **Gene Set** | 多基因平均 log1p 分數渲染為細胞輪廓，適合觀察特定細胞群（如 CD4 T cell）的空間分布 |

#### 預設基因集

內建 8 組常見 TME 基因集（CD4 T、CD8 T、NK、Macrophage、B cell、Tumor、Fibroblast、Endothelial），採 **case-insensitive 匹配**，自動適配人類（`CD4`）與小鼠（`Cd4`）基因命名。

#### Merge ROI 支援

若分析為 merge ROI 模式，探索器自動依 `obs["roi"]` 分組，每個 ROI 獨立渲染為 grid subplot（上限 12 格）。

**輸出**：300 DPI PNG（瀏覽器直接下載），檔名含 ROI、基因集名稱、模式。
...
9. **空間基因表達探索器**：Contour 模式以 skimage `find_boundaries` + RGBA imshow 雙層疊圖呈現細胞填色與邊界；Gene Set 模式支援多基因平均分數一圖呈現；`_parse_cell_id` 嚴格 regex 驗證 obs_name 格式，非 `cell_N` 格式（barcode）提前報錯而非靜默產生錯誤結果。
